...
import (
	...
	"github.com/nousresearch/hermes-go/pkg/security"
)
...
func RegisterWebSearchTool(registry *Registry) error {
	return registry.Register(&handlerTool{
		name:        "web_search",
		...
		schema: DefaultSchema("web_search", "Search the web for information", map[string]any{
		...
		handler: func(args map[string]any) string {
			...
			if !ok {
				return toolResultJSON(false, nil, "query argument is required and must be a string")
			}
			if len(query) > 500 {
				return toolResultJSON(false, nil, "query too long (max 500 characters)")
			}
			query = strings.TrimSpace(query)
			if query == "" {
				return toolResultJSON(false, nil, "query cannot be empty")
			}
			results, err := performWebSearch(query)
			if err != nil {
				return toolResultJSON(false, nil, fmt.Sprintf("search error: %v", err))
			}
			if len(results) == 0 {
				return toolResultJSON(false, nil, "no results found")
			}
